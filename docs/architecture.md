...
建议后续实现时采用分层结构，而不是把所有 Docker 调用写在节点类里。
...
- 先以 Docker 官方 `@docker/node-sdk` 做 POC
- 如果在 `exec`、`archive`、长流处理上遇到工程性问题，再评估切到 `dockerode`

这样设计的原因：

- 官方 SDK 对 `unix`、`tcp`、`tls`、`ssh` 的连接覆盖面更完整
- 类型体验更好
- 但它还比较年轻，不适合在第一天就把整个实现深度绑死

所以最好把 transport 层抽象出来，避免将来切换 SDK 时影响节点层参数设计。
